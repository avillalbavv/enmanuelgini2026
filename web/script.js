...
/* Cuenta regresiva (Inicio) — hasta domingo 7 de junio (internas partidarias) */
(() => {
  const el = document.getElementById("countdown");
  if (!el) return;

  // Domingo 7 de junio de 2026 — 07:00 (hora local) para evitar confusión de medianoche.
  const target = new Date(2026, 5, 7, 7, 0, 0);
  ...
  const render = () => {
    ...
    if (diff <= 0) {
      el.innerHTML = `<span class="cdLabel">Internas partidarias:</span> <span class="cdNum">¡Hoy es el día!</span>`;
      return;
    }
    ...
    el.innerHTML = `
      <span class="cdLabel">Internas partidarias:</span>
      <span class="cdNum">${days}d ${pad2(hours)}:${pad2(mins)}:${pad2(secs)}</span>
      <span class="cdLabel">(Dom. 7 de junio)</span>
    `;
  };
...
(() => {
  ...
  const map = {
    "index.html": "INICIO",
    "biografia.html": "BIOGRAFÍA Y TRAYECTORIA",
    "simulacion.html": "PRACTICÁ TU VOTO",
...
(() => {
  ...
  const introPadron = document.getElementById("introPadron");
  ...
  introPadron?.addEventListener("click", () => close(() => {
    const target = document.getElementById("padron-home");
    if (!target) return;
    if (history && history.replaceState) history.replaceState(null, "", "#padron-home");
  ...
  if (track) {
    ...
    const render = () => {
      ...
      items.forEach(p => {
        ...
        const isIg = platform === "instagram" && !!p.url;
        if (isIg) {
          // ✅ Embed inline (sin botones) + carrusel estable.
          // Nota: el embed de IG requiere https://www.instagram.com/embed.js (ya está cargado en el HTML).
          const card = document.createElement("div");
          card.className = `postTile ${platform} igPost`;
          ...
          card.innerHTML = `
            <div class="pTop">
              <span class="pPlat">${ICONS.instagram || ""}<span>Instagram</span></span>
              <span class="pDate">${fmtDate(p.date)}</span>
            </div>
            <div class="igEmbedWrap" data-processed="0">
      ...
      // Instagram: procesamos embeds de forma lazy para evitar "blanco" cuando el slide no está listo.
      activateIgSlide(0);
    };

    // ───────── Instagram embeds: lazy + pausar video al cambiar ─────────
    let activeIgIndex = -1;

    const igSlides = () => Array.from(track.querySelectorAll('.igPost'));

    const buildBlockquote = (permalink) =>
      `<blockquote class="instagram-media" data-instgrm-permalink="${permalink}" data-instgrm-version="14" style="margin:0; width:100%;"></blockquote>`;

    const resetIgSlide = (idx) => {
      const slides = igSlides();
      const el = slides[idx];
      if (!el) return;
      const wrap = el.querySelector('.igEmbedWrap');
      if (!wrap) return;
      // Reescribir el blockquote mata cualquier reproducción previa (corta el iframe anterior)
      const permalink = el.dataset.permalink || '';
      wrap.innerHTML = buildBlockquote(permalink);
      wrap.dataset.processed = "0";
    };

    const processInstagramEmbeds = () => {
      // Intenta varias veces porque embed.js a veces tarda en cargar.
      let tries = 0;
      const tick = () => {
        tries++;
        ...
        if (tries < 10) setTimeout(tick, 250);
      };
      tick();
    };

    const activateIgSlide = (idx) => {
      const slides = igSlides();
      if (!slides.length) return;
      const safeIdx = Math.max(0, Math.min(idx, slides.length - 1));

      if (activeIgIndex !== -1 && activeIgIndex !== safeIdx) {
        resetIgSlide(activeIgIndex);
      }
      activeIgIndex = safeIdx;

      const wrap = slides[safeIdx]?.querySelector('.igEmbedWrap');
      if (!wrap) return;
      if (wrap.dataset.processed !== "1") {
        processInstagramEmbeds();
        // Marcamos como procesado después de un pequeño delay
        setTimeout(() => { wrap.dataset.processed = "1"; }, 600);
      }
    };
    ...
    // Activar (y re-procesar) el embed del slide visible; al cambiar, corta el video anterior.
    let scrollT = null;
    const onScroll = () => {
      if (scrollT) clearTimeout(scrollT);
      scrollT = setTimeout(() => {
        const step = stepSize() || track.clientWidth || 1;
    ...
    // Si el usuario toca dentro del embed, también pausamos el auto-scroll.
    track.addEventListener('pointerdown', (e) => {
      if (e.target && (e.target.closest?.('.igEmbedWrap') || e.target.closest?.('.instagram-media'))) {
        stopAuto();
      }
    }, { passive:true });
